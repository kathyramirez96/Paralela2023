...
void IMAGENES::RUN(vector<vector<int>>  imagen, int min, int max) const
{
    int detener = 0;
    //string image_path = cv::samples::findFile("/home/user/PARALELA/pro1/sky.png");
    ...
    if(detener == 0){
        ...
        // IMPRIMIR LA ESCALA DE GRISES
            ...
            for(row = rows - 1; row >= 0; --row){
                    for(col = 0; col < cols; ++col){
                        toneLUT[(u_int8_t)imagen[row][col]] = (u_int8_t)imagen[rows][cols]* row + col * 1;
                    }
             }
